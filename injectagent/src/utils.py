...
class Logger(object):
    ...
    def log(self, string, newline=True, force=False):
        ...
        if self.on or force:
            ...
            sys.stdout.flush()
def get_score(output_file):
    dh_output_file = output_file
    # ds_output_file = output_files['ds']
    # if not os.path.exists(dh_output_file):
    #     os.makedirs(os.path.dirname(dh_output_file), exist_ok=True)
    # if not os.path.exists(ds_output_file):
    #     os.makedirs(os.path.dirname(ds_output_file), exist_ok=True)
    cnt_total = 0
    cnt_invalid = 0
    data = []
